SELECT *
FROM sdy1662.elisa_result
WHERE analyte_preferred IN ('TNFA') AND value_preferred = 35;
...
FROM elisa_result
WHERE analyte_preferred IN ('IL8') AND value_preferred > 50;
...
#CREATE TABLE cito_val_inf_filtr AS
SELECT c.subject_accession, c.biosample_accession, b.study_time_collected, ac.name,
  MAX(IL6) AS IL6,
  MAX(IL8) AS IL8,
  MAX(TNFA) AS TNFA,
  MAX(IL1B) AS IL1B
FROM cito_val_inf c, arm_2_subject a, arm_or_cohort ac, biosample b
WHERE c.subject_accession=a.subject_accession AND a.arm_accession = ac.arm_accession AND c.biosample_accession=b.biosample_accession
GROUP BY c.biosample_accession;

CREATE TABLE cito_val_inf AS
SELECT subject_accession, biosample_accession, study_time_collected,
  CASE WHEN analyte_preferred = 'IL6' THEN value_preferred ELSE NULL END AS IL6,
  CASE WHEN analyte_preferred = 'IL8' THEN value_preferred ELSE NULL END AS IL8,
  CASE WHEN analyte_preferred = 'TNFA' THEN value_preferred ELSE NULL END AS TNFA,
...
#CREATE TABLE lymphocites AS
SELECT biosample_accession, 
	MAX(CASE WHEN name_preferred = 'Lymphocyte' THEN l.result_value_preferred ELSE NULL END) AS 'Lymphocyte', 
	MAX(CASE WHEN name_preferred = 'Lymphocyte % of WBC' THEN l.result_value_preferred ELSE NULL END) AS 'Lymphocyte_percentage_WBC'
FROM lab_test
WHERE name_preferred LIKE 'Lymphocyte%' AND l.result_value_preferred IS NOT NULL AND a.result_value_reported LIKE 'DETECTED'
GROUP BY biosample_accession;

#CREATE TABLE cito_elevado_final AS
SELECT c.subject_accession, c.biosample_accession, c.study_time_collected, c.name, c.IL6, c.IL8, c.TNFA, c.IL1B, l.Lymphocyte, l.Lymphocyte_percentage_WBC
FROM cito_val c, lymphocites l, biosample b
WHERE l.biosample_accession = b.biosample_accession AND b.subject_accession = c.subject_accession;

CREATE TABLE cito_val_no_filtr_final AS
SELECT c.subject_accession, c.biosample_accession, c.study_time_collected, c.IL6, c.IL8, c.TNFA, c.IL1B, l.Lymphocyte, l.Lymphocyte_percentage_WBC
FROM cito_val_no_filtr c, lymphocites l, biosample b
WHERE c.subject_accession = b.subject_accession AND b.biosample_accession = l.biosample_accession;

#TABLA CON TODOS LOS DATOS SIN FILTRAR SI ESTÁN POR ENCIMA O POR DEBAJO DEL UMBRAL
CREATE TABLE cito_val_no_filtr_2 AS
